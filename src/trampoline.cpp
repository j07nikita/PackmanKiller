...
Trampoline::Trampoline(float x, float y, color_t color) {
    ...
    vertex_buffer_data[s++] = -0.6;
    vertex_buffer_data[s++] = 0;
    vertex_buffer_data[s++] = 0;
    vertex_buffer_data[s++] = -0.5;
    vertex_buffer_data[s++] = 0;
    vertex_buffer_data[s++] = 0;
    vertex_buffer_data[s++] = -0.5;
    vertex_buffer_data[s++] = -1;
    vertex_buffer_data[s++] = 0;
    vertex_buffer_data[s++] = -0.5;
    vertex_buffer_data[s++] = -1;
    vertex_buffer_data[s++] = 0;
...
bool Trampoline::on_trampoline(bounding_box_t a) {
    return ((a.x <= this->position.x + 0.6) && (a.x >= this->position.x - 0.6) && (a.y - this->position.y <= a.width));
}
